  ...
  return (
    <div className="w-full max-w-7xl mx-auto bg-gradient-to-r from-white/80 via-emerald-50/80 to-sky-50/80 backdrop-blur-lg rounded-2xl shadow-lg border border-gray-200/50 p-3 px-6 flex items-center justify-between">
      ...
      <div className="hidden lg:flex items-center space-x-6">
        {kpis.map((kpi) => (
          <div key={kpi.title} className="text-center cursor-pointer group" onMouseEnter={() => setHoveredKpi(kpi.title)} onMouseLeave={() => setHoveredKpi(null)}>
            <div className="flex items-center justify-center space-x-2">
                <kpi.icon size={16} className={`transition-colors duration-300 ${hoveredKpi === kpi.title ? kpi.hoverColor : 'text-gray-500'}`} />
                <p className={`text-lg font-semibold transition-colors duration-300 ${hoveredKpi === kpi.title ? 'text-gray-900' : 'text-gray-800'}`}>{kpi.value}</p>
            </div>
            <p className={`text-[10px] font-light transition-colors duration-300 ${hoveredKpi === kpi.title ? kpi.hoverColor : 'text-gray-500'}`}>{kpi.title} ({kpi.unit})</p>
          </div>
        ))}
      </div>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="bg-white/50 border-gray-200/50 rounded-lg font-light text-gray-700 text-xs px-3 py-1 h-auto transition-colors hover:bg-white/80 hover:text-gray-900">
            Unidades
            <ChevronDown className="ml-2 h-3 w-3" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="w-64 bg-white/70 backdrop-blur-lg border-gray-200/50 rounded-lg text-black" align="end">
          {Object.entries(unitsByCountry).map(([country, countryUnits], index) => (
            <DropdownMenuGroup key={country}>
              ...
              {index < Object.keys(unitsByCountry).length - 1 && (<DropdownMenuSeparator className="bg-gray-200/50" />)}
            </DropdownMenuGroup>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
